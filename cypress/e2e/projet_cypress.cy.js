...
describe('Site Magento', function(){
    ...
    it(' Create an Account', function(){
        ...
        cy.get('#password').type('Azerty123').should('have.value','Azerty123')
        cy.get('#password-confirmation').type('Azerty123').should('have.value','Azerty123')
    ...
    it('Sign in', () => {
        ...
        cy.get('#send2').click()
        cy.wait(1000)
        cy.get('.page-header').find('.logged-in').should('have.text',"Welcome, "+firstname+" "+lastname+"!")
        cy.wait(2000)
        cy.get('[data-role="content"]').find('#ui-id-8').should('have.text','Sale').click()
        cy.get('.page-title-wrapper').find('[data-ui-id="page-title-wrapper"]').should('have.text','Sale')
        cy.get('.categories-menu').find('.title').eq(2).should('have.text',"Gear Deals")
        cy.get('.categories-menu').find('.item :nth-child(1)').eq(11).click()
        cy.get('.products.list').find('.item.product').eq(2).click()
        cy.wait(1000)
        cy.get('#product-addtocart-button').click()
        cy.wait(1000)
        cy.get('.message-success').should('contain.text','You added Driven Backpack to your shopping cart')
        cy.wait(1000)
